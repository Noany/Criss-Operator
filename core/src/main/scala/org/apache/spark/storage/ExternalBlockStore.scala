...
import tachyon.thrift.{BenefitInfo, ClientFileInfo}
...
private[spark] class ExternalBlockStore(blockManager: BlockManager, executorId: String)
  ...
  def setBenefit(benefit: collection.mutable.Map[Int, BenefitInfo]) =
    externalBlockManager.map(_.setBenefit(benefit))
  ...
  def checkGlobalExists(blockId: BlockId): Boolean = {
    externalBlockManager.map(_.checkGlobalExists(blockId)).getOrElse(false)
  }
  */
  ...
  def checkGlobalExists(operatorId: Int): Boolean = {
    externalBlockManager.map(_.checkGlobalExists(operatorId)).getOrElse(false)
  }
  ...
  def removeGlobalFiles(operatorId: Int) = {
    try {
      externalBlockManager.map(_.removeGlobalFiles(operatorId))
  ...
  def listStatus(operatorId: Int): List[ClientFileInfo] = {
    try {
      externalBlockManager.map(_.listStatus(operatorId)).
        getOrElse(new util.ArrayList[ClientFileInfo]())
    } catch {
      case NonFatal(t) =>
        logError(s"Error in getBytes($operatorId)", t)
        new util.ArrayList()
