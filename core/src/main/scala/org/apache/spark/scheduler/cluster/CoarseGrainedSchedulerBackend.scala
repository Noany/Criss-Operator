  ...
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def receive: PartialFunction[Any, Unit] = {
      ...
      //zengdan
      case SendBenefit(benefit) =>
        logInfo("Send " + benefit.size + " Benefits to Executors: ")
        executorDataMap.foreach(_._2.executorEndpoint.send(SendBenefit(benefit)))
  ...
  override def sendBenefit(benefit: scala.collection.mutable.Map[Int, BenefitInfo]) {
    driverEndpoint.send(SendBenefit(benefit))
  }
