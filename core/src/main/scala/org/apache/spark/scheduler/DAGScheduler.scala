...
import java.io.{InputStreamReader, BufferedReader, NotSerializableException}
...
class DAGScheduler(
  ...
  //zengdan
  private[scheduler] val queryStatistics = scala.collection.mutable.Map[Int, Array[Long]]()
  //private[scheduler] val shuffleRemoteReadBytes = scala.collection.mutable.Map[String, Long]()
  ...
  def taskEnded(
      ...
      taskStats: Option[scala.collection.mutable.Map[Int,Array[Int]]] = None): Unit = {
    eventProcessLoop.post(
      CompletionEvent(task, reason, result, accumUpdates, taskInfo, taskMetrics, taskStats))
  }
  ...
  def runJob[T, U](
    ...
    //shuffleRemoteReadBytes.foreach{x => logError("executor: " + x._1 + " shuffleRemoteRead: " + x._2)}
  }
      ...
      case Success =>
        ...
        task match {
          ...
          case smt: ShuffleMapTask =>
            ...
            if(stage.id == 11 && shuffleReadMetrics.isDefined) {
              shuffleRemoteReadBytes.put(execId,
                shuffleRemoteReadBytes.get(execId).getOrElse(0L) + shuffleReadMetrics.get.remoteBytesRead)
            }
            //zd
            */
