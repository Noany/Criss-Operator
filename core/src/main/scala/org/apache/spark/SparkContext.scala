...
import org.apache.hadoop.mapred.{JobConf, TextInputFormat, SequenceFileInputFormat, FileAlreadyExistsException}
import org.apache.hadoop.mapred.{InputFormat, FileInputFormat}
...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  def setBenefit(benefit: scala.collection.mutable.Map[Int, BenefitInfo]) = {
    this.taskScheduler.sendBenefit(benefit)
  }
  ...
  */
  ...
  def loadCachedFile[U:ClassTag](operatorId: Int):Option[(RDD[U])] = {
    ...
    val path = tachyonUrl + rootPath + "/" + operatorId
    val tachyonStore = SparkEnv.get.blockManager.externalBlockStore
    if (TachyonBlockManager.canReuseFile(operatorId)) {
  ...
  def getGlobalPath(operatorId: Int): String = {
    val tachyonUrl = conf.get("spark.tachyonStore.url", "tachyon://localhost:19998")
    val rootPath = conf.get("spark.tachyonStore.global.baseDir", "/global_spark_tachyon")
    tachyonUrl + rootPath + "/" + operatorId
  }
  ...
  val sparkUser = org.apache.spark.util.Utils.getCurrentUserName()
